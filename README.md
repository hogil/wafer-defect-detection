...
### **시나리오 A: 쉬운 클래스 처리**
```
📸 이미지 입력 → 🤖 기본 분류 → ✅ 높은 신뢰도 → 즉시 결과 반환
```

**특징:**
- **1단계만** 사용하여 빠른 처리
- 높은 성능 클래스는 추가 검증 불필요
- 전체 시스템 **효율성 극대화**

### **시나리오 B: 어려운 클래스 정밀 분석**
```
📸 이미지 입력 → 🤖 기본 분류 → ⚠️ 낮은 신뢰도 → 🎯 ROI 검증 → ✅ 최종 결과
```

**정밀 분석 과정:**
1. **조건 확인**: 어려운 클래스 + 낮은 신뢰도
2. **ROI 적용**: 사전 학습된 관심영역 패턴 사용
3. **객체 검출**: YOLO를 통한 ROI 내 객체 분석
4. **역매핑**: 검출된 객체로부터 최종 클래스 결정

### **시나리오 C: 클래스 재분류**
```
🔄 초기 예측: "scratch" → 🎯 ROI 분석 → 🔍 "blob" 객체 다수 검출 → ✨ 최종: "contamination"
```

**지능적 오류 수정:**
- 초기 분류 결과에 **의존하지 않음**
- 객체 증거를 바탕으로 **논리적 재판단**
- 모델의 한계를 **데이터 기반 검증**으로 보완
